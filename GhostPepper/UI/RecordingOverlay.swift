...
enum OverlayMessage: Equatable {
    ...
    var primaryText: String {
        ...
        case .completed(let seconds):
            return String(format: "Done (%.1fs)", seconds)
        }
    }
...
class RecordingOverlayController {
    ...
    private func scheduleDismissIfNeeded(for message: OverlayMessage) {
        switch message {
        case .clipboardFallback, .learnedCorrection, .noSoundDetected, .completed:
            let delay: TimeInterval
            switch message {
            case .noSoundDetected: delay = 5
            case .completed: delay = 1.5
            default: delay = 3
            }
