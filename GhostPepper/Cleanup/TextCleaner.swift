...
final class TextCleaner {
    ...
    **英文专有名词:**
    ...
    # 长度硬约束(覆盖所有其他指令,大模型务必严格遵守)

    - 输出长度必须**约等于输入长度**(允许 ±30% 浮动,常用于断句标点和填充词清理)
    - **绝不**扩写、展开、补充、添加用户没说过的内容
    - **绝不**把"整理"理解成"完善"或"写得更完整"
    - **绝不**主动添加解释、上下文、礼貌用语、过渡句
    - 如果不确定,**输出更少**而不是更多
    - 如果用户只说了一句话,输出就只有一句话(不是一段)
    - 如果用户说话很短,输出就很短
