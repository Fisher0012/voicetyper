    ...
    func transcribe(audioBuffer: [Float], language: String? = nil) async -> String? {
        ...
        do {
            ...
            case .whisperKit:
                ...
                // 移植自 voicetyper:贪心解码(temperature=0)更快更稳定。
                decodeOptions.temperature = 0.0
                // 注意:绝不要往 promptTokens 塞领域词典 / 术语列表!实测发现 WhisperKit
                // 会把 promptTokens 当成"刚才说过的上下文"做生成偏置,塞词典反而触发严重
                // hallucination 与整句重复(用户说"你又在真正听我说吗"被识别成"以久在甚真
                // 听我说吗"且重复 2 次)。术语精度由 LLM cleanup 层(TextCleaner)的用户字典
                // + few-shot prompt 处理 — 那才是设计正确的注入点。
                if let language {
