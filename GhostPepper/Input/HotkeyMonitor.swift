...
final class HotkeyMonitor: NSObject, HotkeyMonitoring {
    ...
    // 新路径:NSEvent global monitor — 不需要 Input Monitoring 权限,不会被 macOS 周期 disable
    ...
    /// Starts monitoring for key chord events using NSEvent global monitor.
    /// - Returns: `false` if monitor creation fails (typically because Accessibility is denied).
    func start() -> Bool {
        stateLock.lock()
        if globalEventMonitor != nil {
            stateLock.unlock()
            debugLogger?(.hotkey, "Hotkey monitor start skipped because monitor is already active.")
            return true
        }
        stateLock.unlock()

        // NSEvent global monitor 监听全局修饰键变化事件:
        // - 只需 Accessibility 权限(不需要 Input Monitoring)
        // - 不会被 macOS 周期性 disable(CGEventTap 在无 Input Monitoring 时的痛点)
        // - 替换 .app 文件后 Accessibility 授权也会丢,但用户只需在系统设置授权一次即可
        let monitor = NSEvent.addGlobalMonitorForEvents(matching: [.flagsChanged]) { [weak self] event in
            self?.handleNSEvent(event)
        }

        guard let monitor else {
            debugLogger?(.hotkey, "Hotkey monitor failed: NSEvent.addGlobalMonitorForEvents returned nil (Accessibility permission likely denied).")
            return false
        }
        ...
        stateLock.unlock()
        debugLogger?(.hotkey, "Hotkey monitor started via NSEvent global monitor.")
        return true
    }

    /// Stops monitoring and removes the NSEvent global monitor.
    func stop() {
        stateLock.lock()
    ...
    /// 处理来自 NSEvent global monitor 的事件,转换为 EventSnapshot 后交给原有状态机。
