...
final class TextPaster {
    ...
    func paste(text: String) -> PasteResult {
        ...
        // 无脑强制 Cmd+V(对齐 voicetyper Python 版行为)。
        // 不做"焦点元素是否可粘贴"的预检 — Accessibility 对 Cursor/终端/Electron/Notion/飞书等
        // 兼容性不一,预检会误判为不可粘贴,导致用户在那些场景按热键没文字出现。
        // 副作用:在确实不能粘的窗口(如 Finder)发送空 Cmd+V,通常无害(系统会忽略或滴一声)。
    ...
    private static func defaultCommandVPasteAction() -> (() -> Void)? {
        // 标准 CGEvent.post 路径模拟 Cmd+V。需要 app 自身有 Accessibility 权限,前提是
        // app 安装在 /Applications/ 等标准位置 + 用 cert 签名(非 ad-hoc),这样 TCC 授权
        // 才会按 Team ID + bundle id 持久化,不会因 .app 文件替换而失效。
